...
namespace NetCoreMVCEgitimi.Areas.ApiKullanimi.Controllers
{
    ...
    public class UyelerController : Controller
    {
        ...
        string _url = "https://localhost:7153/api/uyeler/";
        ...
        {
            var model = await _httpClient.GetFromJsonAsync<List<Uye>>(_url + id);
            return View(model);
        }
        ...
        public async Task<ActionResult> Create(Uye collection)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var response = await _httpClient.PostAsJsonAsync(_url, collection);
                    if (response.IsSuccessStatusCode) // post metodu cevap olarak geriye değer döndürüyor.
                        return RedirectToAction(nameof(Index)); // eğer api de başarılı kodu dönmüşse
                }
                catch
                {
